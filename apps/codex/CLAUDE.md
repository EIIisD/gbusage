...
- Treat Codex as a sibling to `apps/gbusage`; whenever possible reuse the same shared packages (`@gbusage/terminal`, pricing helpers, logging), command names, and flag semantics. Diverge only when Codex-specific data forces it and document the reason inline.
