 ...
 * for gbusage configuration files. The generated schema enables:
...
function createConfigSchemaJson() {
	...
	return {
		$ref: '#/definitions/gbusage-config',
		definitions: {
			'gbusage-config': {
				type: 'object',
				properties: {
		...
		title: 'gbusage Configuration',
		description: 'Configuration file for gbusage - Claude Code usage analysis tool',
		examples: [
			{
...
if (import.meta.vitest != null) {
	...
	describe('createConfigSchemaJson', () => {
		it('should generate valid JSON Schema', () => {
			...
			expect(jsonSchema.$ref).toBe('#/definitions/gbusage-config');
			expect(jsonSchema.definitions).toBeDefined();
			expect(jsonSchema.definitions['gbusage-config']).toBeDefined();
			expect(jsonSchema.definitions['gbusage-config'].type).toBe('object');
		});

		it('should include all expected properties', () => {
			const jsonSchema = createConfigSchemaJson();
			const mainSchema = jsonSchema.definitions['gbusage-config'];
		...
		it('should include all command schemas', () => {
			const jsonSchema = createConfigSchemaJson();
			const commandsSchema = jsonSchema.definitions['gbusage-config'].properties.commands;
