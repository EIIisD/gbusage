...
if (import.meta.vitest != null) {
	...
	describe('loadConfig', () => {
		...
		it('should handle invalid JSON gracefully', async () => {
			await using fixture = await createFixture({
				'.gbusage/gbusage.json': '{ invalid json }',
			});
		...
		it('should prioritize local .gbusage config over Claude paths', async () => {
			await using fixture = await createFixture({
				'.gbusage/gbusage.json': JSON.stringify({
					defaults: { json: true },
		...
		it('should test configuration priority order with multiple files', async () => {
			...
			expect(config1?.source).toBe(fixture.getPath('.gbusage/gbusage.json'));
			...
			expect(config2).toBeUndefined(); // No local .gbusage and no real Claude paths
		});
		...
		it('should handle getClaudePaths() errors gracefully', async () => {
			...
			const config = loadConfig();
			expect(config).toBeDefined();
			expect(config?.source).toBe(fixture.getPath('.gbusage/gbusage.json'));
			expect(config?.defaults?.json).toBe(true);
		});
		...
		it('should use validateConfigFile internally', async () => {
			...
			const validResult = validateConfigFile(fixture.getPath('.gbusage/gbusage.json'));
	...
	describe('debug functionality', () => {
		...
		describe('loadConfig with debug', () => {
			it('should log debug info when loading config with debug=true', async () => {
				...
				expect(loggerInfoSpy).toHaveBeenCalledWith(`  • Checking: ${fixture.getPath('.gbusage/gbusage.json')} (found ✓)`);
				expect(loggerInfoSpy).toHaveBeenCalledWith('');
				expect(loggerInfoSpy).toHaveBeenCalledWith(`Loaded config from: ${fixture.getPath('.gbusage/gbusage.json')}`);
				expect(loggerInfoSpy).toHaveBeenCalledWith('  • Schema: https://gbusage.com/config-schema.json');
