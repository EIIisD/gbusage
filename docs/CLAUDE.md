...
This directory contains the VitePress-based documentation website for gbusage.

## Package Overview

**Name**: `@gbusage/docs`
**Description**: Documentation for gbusage
...
- `public/config-schema.json` - JSON schema copied from gbusage package during build
...
1. **API Documentation**: `./update-api-index.ts` generates API docs from gbusage TypeScript source
2. **Schema Copy**: `config-schema.json` is copied from the gbusage package to public directory
...
- `gbusage` - Main package (workspace dependency for API docs)
