...
> ⚠️ The Codex companion CLI is experimental. Expect breaking changes while both gbusage and [OpenAI's Codex CLI](https://github.com/openai/codex) continue to evolve.

The `@gbusage/codex` package reuses gbusage's responsive tables, pricing cache, and token accounting to analyze OpenAI Codex CLI session logs.
...
pnpm dlx @gbusage/codex --help
pnpx @gbusage/codex --help
...
Since `npx @gbusage/codex@latest` is quite long to type repeatedly, we strongly recommend setting up a shell alias for convenience:
...
- Additional reports will mirror the gbusage CLI as the Codex tooling stabilizes.
