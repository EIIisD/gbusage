...
gbusage supports three different cost calculation modes to handle various scenarios and data sources. Understanding these modes helps you get the most accurate cost estimates for your usage analysis.

## Overview

Claude Code stores usage data in JSONL files with both token counts and pre-calculated cost information. gbusage can handle this data in different ways depending on your needs:
...
gbusage session --mode display --json
```
...
gbusage daily --mode auto --since 20241201
...
gbusage daily --mode display --since 20250101
...
gbusage daily --mode auto --since 20240101 --until 20241231

# Calculate mode for consistent comparison
gbusage monthly --mode calculate --order asc
```
...
When calculating costs from tokens, gbusage uses:
...
gbusage daily --mode calculate --breakdown

# Display mode with JSON output for analysis
gbusage session --mode display --json | jq '.[] | select(.totalCost > 0)'

# Auto mode with date filtering
gbusage monthly --mode auto --since 20240101 --order asc
```
...
gbusage daily --mode calculate --offline
gbusage monthly --mode auto --offline
```
...
gbusage daily --mode calculate --since 20240101
```
...
gbusage daily --mode auto  # Updates pricing cache
gbusage daily --mode calculate --debug  # Compare calculations
```
