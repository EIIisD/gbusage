...
The gbusage MCP server now lives in the dedicated `@gbusage/mcp` package. This keeps the main CLI lightweight while still giving you full access to MCP tools for daily, session, monthly, and billing-block analytics.
...
```bash
bunx @gbusage/mcp@latest --help
# or
npx @gbusage/mcp@latest --help
```

All examples below use `bunx @gbusage/mcp@latest` (you can substitute with `npx @gbusage/mcp@latest` if preferred).
...
```bash
bunx @gbusage/mcp@latest
# equivalent:
bunx @gbusage/mcp@latest --type stdio
```
...
```bash
bunx @gbusage/mcp@latest --type http --port 8080
```
...
Each tool accepts `since`, `until`, and `mode` parameters, plus timezone/locale overrides identical to the gbusage library.
...
```bash
bunx @modelcontextprotocol/inspector bunx @gbusage/mcp@latest
# or
npx @modelcontextprotocol/inspector npx @gbusage/mcp@latest
```
...
- "Ask the gbusage MCP server for today's usage report"
...
```ts
import { createMcpServer } from '@gbusage/mcp';
