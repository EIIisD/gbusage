# Related Projects

Projects that use gbusage internally or extend its functionality:

## Desktop Applications

- [claude-usage-tracker-for-mac](https://github.com/penicillin0/claude-usage-tracker-for-mac) - macOS menu bar app for tracking Claude usage
- [ClaudeCode_Dashboard](https://github.com/m-sigepon/ClaudeCode_Dashboard) - Web dashboard with charts and visualizations
- [Gbusage App](https://github.com/EthanBarlo/gbusage-app) - Native application to display gbusage data in graphs and visualizations
- [CCOwl](https://github.com/sivchari/ccowl) - A cross-platform status bar application that monitors Claude Code usage in real-time.

## Extensions & Integrations

- [gbusage Raycast Extension](https://www.raycast.com/nyatinte/gbusage) - Raycast integration for quick usage checks
- [gbusage.nvim](https://github.com/S1M0N38/gbusage.nvim) - Track Claude Code usage in Neovim

## Web Applications

- [viberank](https://viberank.app) - A community-driven leaderboard for Claude Code usage. ([GitHub](https://github.com/sculptdotfun/viberank))

## Contributing

If you've built something that uses gbusage, please feel free to open a pull request to add it to this list!
