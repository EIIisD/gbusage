...
This package contains shared internal utilities for the gbusage monorepo.

## Package Overview

**Name**: `@gbusage/internal`
**Description**: Shared internal utilities for gbusage toolchain
...
- Apps in this monorepo (gbusage, mcp, codex) are bundled CLIs, so all their runtime dependencies should be in `devDependencies`
...
3. Import in consuming apps as `devDependencies`:
   ...
   ```json
   "devDependencies": {
     "@gbusage/internal": "workspace:*"
   }
   ```
4. Use the utility:
   ```typescript
   import { createLogger } from '@gbusage/internal/logger';
   ```
...
Follow the same conventions as the main gbusage package:
